...
func main() {
	...
	*/
	...
	for i := 0; i < 15; i++ {
		...
		//길이 및 용량 자동 증가(용량 : 2배 - 5 10 20 40 이렇게 *2 하는 방법으로 증가됨)
	}
	...
	*/
	...
	[나의 착각으로 정정 : make 는 새로운 객체나 메모리주소를 생성하지 않음]
	...
	*/
	...
	// [slice는 기본적으로 reference니까]
	// 정정 : 위에서 내가 착각한 부분이 있었어 테스트 해보니, make는 객체를 만들거나 새로운 메모리주소를 할당하는 메소드가 아니야!
	...
	f := e[0:5:7] //용량 지정, 마지막 인덱스가 용량을 뜻하는 용량지정임, 파이썬에서 슬라이스 Step과 다르다는 것을 유의
	...
	fmt.Println("ex4 : ", e)
	fmt.Println("ex4 : ", len(f), cap(f))
	fmt.Println("ex4 : ", f) // 그렇지 주소만 가져온거야 근데 용량이 7이라는 건 신기하네[위의 짧은 선언 때 용량지정함]
