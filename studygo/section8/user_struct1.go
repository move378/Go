...
/*
	구조체 <-> 메소드 바인딩 [어떤 구조체(속성, 상태)에 기능을 추가하는 것]

구조체에 대한 주소 값이 있고, 그와 연결된 메소드가 있는 형태인가?
메소드 바인딩: func (매개변수로 구조체) 메소드명(매개변수) 반환형 {} 형태
...
func main() {
	...
	구조체와 -> 메서드 연결(바인딩)을 통해서 타 언어의 클래스 형식 처럼 사용 가능(객체 지향)
	*/
	//예제1
	...
	fmt.Println("ex1 : ", benz.Price()) // .으로 접근하여 객체지향 식의 사용방법
